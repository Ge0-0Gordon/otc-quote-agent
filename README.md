...
正式开发计划见 [docs/development_plan.md](docs/development_plan.md)，优化实施计划见
[docs/optimization_implementation_plan.md](docs/optimization_implementation_plan.md)。

## 功能范围

- 输入：文本粘贴、TXT、Markdown、DOCX、XLSX、文本型 PDF、EML。
- 产品：Snowball、FCN、European Vanilla Option。
- 处理：规则优先分类、Schema 选择、LLM 抽取、字段标准化、业务校验和复核问题生成。
- 输出：标准 JSON、CSV 报价表和自包含 HTML 报告；多方案询价按候选行展示。
...
推理较慢的模型（例如部分百炼 DeepSeek 服务）可将
`LLM_TIMEOUT_SECONDS` 提高到 `300`。不建议禁用超时，否则 Provider
挂起时 CLI 和 Streamlit 会无限等待。
...
页面支持文本粘贴、文件上传、报价表格、规范 JSON、质量报告、字段来源、
人工修正后重新校验，以及三个下载按钮。人工复核不会再次调用 LLM。JSON
中百分比使用小数，例如 `1.03`；页面和 HTML 报告显示为 `103%`。
...
`evaluate_samples.py` 使用固定 LLM fixture 检查分类、归一化、校验和
unsupported 行为；它是确定性回归结果，不冒充真实模型准确率。

测试覆盖：

- 六类文件解析、输入安全上限及空文本 PDF 的扫描件提示。
...
- `sample_data/reference_case_09_limited_loss_snowball.txt`
- `sample_data/reference_case_11_dcn_unsupported.txt`
- `sample_data/reference_case_12_sharkfin_unsupported.txt`
- `sample_data/reference_case_13_snowball_two_choices.txt`

样例故意缺少部分字段，用于展示缺失字段、警告和人工复核问题。

## Reference materials

项目已根据官方材料 `given-data/金融创新业务部衍生品参考资料.docx` 完成适配：

- “标准化衍生品结构要素”已提取到 `reference_materials/standard_derivative_terms.yaml`，并映射到 Pydantic schema。
- “相关询价案例”中的案例 7–13 已提取到 `reference_materials/inquiry_cases.yaml`。
- 案例 9、11、12、13 的官方原文已映射到 `sample_data/reference_case_*.txt`。
- 官方案例 1–6 的内嵌图片已经人工核对转录；案例 1–5 为雪球多方案，
  案例 6 的凤凰/DCN 保持 unsupported。详见
  `docs/official_image_cases_inventory.md`。
- 字段映射和产品边界见 `docs/reference_mapping.md`。
- DCN、凤凰、美式鲨鱼鳍和鲨鱼鳍暂不完整支持；系统会识别为 `unsupported`，不套用其他产品 schema，也不伪造报价字段。
...
配置真实 provider 后，也可以运行全部演示样例：

```powershell
.\scripts\run_all_samples.ps1
```
...
- 原文和字段证据随结果保留，便于人工复核。
- `field_metadata` 记录字段来源、抽取方法、归一化状态及系统计算的置信度。
- `processing_metadata.classification_confidence` 与字段置信度分开记录。

## 已知限制

- PDF 仅支持可提取文本的文件；提取结果为空时按疑似扫描件处理，不执行 OCR。
- DOCX、XLSX、PDF 仅做简单文本提取，不重建复杂表格或版面。
- 文件大小、解压后 Office 大小、PDF 页数、XLSX sheet/cell 数量和提取文本
  长度具有确定性上限；超限时显式失败。
- 不支持页码级证据定位。
